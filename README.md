...
## Важно перед публикацией в Git

В репозиторий должны попадать только файлы проекта. Локальные зависимости, сборки, кэши, `.env`, файлы IDE и служебные файлы Codex игнорируются через `.gitignore`.

Открывайте терминал именно в папке проекта:

```bash
cd "C:\Users\Admin\Desktop\уник\3 курс\Рубцова\cyberteka"
```

Если репозиторий ещё не создан, выполните:
...
Перед загрузкой на GitHub проверьте список файлов:

```bash
git status --short
```

В списке не должно быть `node_modules`, `dist`, `.env`, `.idea`, `.codex` и других локальных служебных файлов.
